...
class VerificationCodesController extends BaseController
{
     ...
     */
    public function check(VerificationRequest $request, EasySms $sms): JsonResponse
    {
        $cacheCaptchaInfo = Cache::get($request->get('captcha_key'));
        // 缓存信息校验
        if (!$cacheCaptchaInfo) {
            return $this->responseData(Codes::CODE_CAPTCHA_EXPIRE, [], Codes::STATUS_CODE_UNPROCESSABLE_ENTITY);
        }
        // 判断验证码
        if (!captcha_api_check($request->get('captcha_code'), $cacheCaptchaInfo["captcha_key"], "flat")) {
            Cache::forget($request->get('captcha_key'));
            return $this->responseData(Codes::CODE_CAPTCHA_INVALID, [], Codes::STATUS_CODE_NOT_AUTHORIZATION);
        }

        Cache::forget($request->get('captcha_key')); // 删除缓存的验证码key

        // 短信验证码, 本地不进行正常的发送
        $phone = $cacheCaptchaInfo["phone"];
